import type { ImprovedCardProps } from "./ImprovedCard";

export function ImprovedCardLayout({ items }: ImprovedCardProps) {
  return (
      ...
      <ul className="divide-y divide-dashed divide-[#D2D6DB]">
        {items.map(({ iconSrc, label, delta }) => (
          <li key={label} className="flex items-center gap-3 py-[19px]">
            ...
            <span className="flex items-center gap-1 text-base font-medium text-[#519C66]">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img src="/Trend Icon.svg" alt="" className="h-4 w-4" />+{delta}
            </span>
          </li>
        ))}
      </ul>
